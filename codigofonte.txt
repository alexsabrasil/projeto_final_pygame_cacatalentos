...
Projeto: Game of the Python

Lista dos Participantes do Grupo Python Pygame

Em ordem alfabetica

- Alexsandra Tavares Sabino da Silva
- Emerson José Souza Veira
...
background = pygame.image.load('fundos\cenario.png')
...
maca_image = pygame.image.load('sprites/apple.png')
...
def aumenta_cobra(lista_cobra):
    for tile in range(len(lista_cobra)):
        if (tile == 0 and len(lista_cobra) > 1):  # Rabo
            ...
            snake_head = pygame.image.load('sprites/head_up.png')
            if(lista_cobra[tile][0] > lista_cobra[tile-1][0] and lista_cobra[tile][1] == lista_cobra[tile-1][1]):
